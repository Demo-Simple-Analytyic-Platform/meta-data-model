...
AS BEGIN
  ...
  BEGIN
    ...
    SET @tx_msg = '/* Build, Show and Execute SQL-statement for "creating" the "Procedure". */';
    SET @tx_sql  = @tx_emp + 'CREATE PROCEDURE ' + @tx_prc + ' @ip_is_debugging BIT = 0 AS BEGIN';
    SET @tx_sql += @tx_nwl + '  ';
    SET @tx_sql += @tx_nwl + '  UPDATE t SET';
    SET @tx_sql += @tx_nwl + '    t.meta_is_active = 0,';
    SET @tx_sql += @tx_nwl + '    t.meta_dt_valid_till = ISNULL((SELECT MAX(meta_dt_valid_from) FROM ' + @tx_src + '), GETDATE())';
    SET @tx_sql += @tx_nwl + '  FROM ' + @tx_tgt + ' AS t LEFT';
    SET @tx_sql += @tx_nwl + '  JOIN ' + @tx_src + ' AS s';
    SET @tx_sql += @tx_nwl + '  ON t.meta_ch_rh = s.meta_ch_rh';
    SET @tx_sql += @tx_nwl + '  ';
    SET @tx_sql += @tx_nwl + '  WHERE t.id_model IN (SELECT id_model FROM deployment.current_model)';
    SET @tx_sql += @tx_nwl + '  AND   t.meta_is_active = 1';
    SET @tx_sql += @tx_nwl + '  AND   s.meta_ch_pk IS NULL;';
    SET @tx_sql += @tx_nwl + '  ';
    SET @tx_sql += @tx_nwl + '  INSERT INTO ' + @tx_tgt + ' (';
    SET @tx_sql += @tx_nwl + '    ' + REPLACE(@tx_col, 's.', '') + '';
    SET @tx_sql += @tx_nwl + '    meta_dt_valid_from, meta_dt_valid_till, meta_is_active, meta_ch_rh, meta_ch_bk, meta_ch_pk';
    SET @tx_sql += @tx_nwl + '  )';
    SET @tx_sql += @tx_nwl + '  SELECT ' + @tx_col + '';
    SET @tx_sql += @tx_nwl + '         s.meta_dt_valid_from, s.meta_dt_valid_till, s.meta_is_active, s.meta_ch_rh, s.meta_ch_bk, s.meta_ch_pk';
    SET @tx_sql += @tx_nwl + '  FROM ' + @tx_src + ' AS s LEFT';
    SET @tx_sql += @tx_nwl + '  JOIN ' + @tx_tgt + ' AS t';
    SET @tx_sql += @tx_nwl + '  ON  t.meta_is_active = 1 ';
    SET @tx_sql += @tx_nwl + '  AND t.meta_ch_rh = s.meta_ch_rh ';
    SET @tx_sql += @tx_nwl + '  AND t.id_model IN (SELECT id_model FROM deployment.current_model)';
    SET @tx_sql += @tx_nwl + '  ';
