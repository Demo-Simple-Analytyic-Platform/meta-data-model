...
BEGIN
  ...
  IF (1=1 /* Build SQL Statemen for creation of "Stored Procedure" */) BEGIN
    ...
    SET @tx_sql += @nwl + '            SET @tx_error_message  = "ERROR: meta_ch_pk NOT unique for ' + @ip_nm_target_schema + '.' + @ip_nm_target_table + '!" + CHR(10) + "-- SQL Statement:"'
    SET @tx_sql += @nwl + '            SET @tx_error_message += CHR(10) + "SELECT * FROM ' + @tsa + '"'
    SET @tx_sql += @nwl + '            SET @tx_error_message += CHR(10) + "WHERE meta_ch_pk IN (SELECT meta_ch_pk FROM ' + @tsa + ' GROUP BY meta_ch_pk HAVING COUNT(*) > 1);"'
    SET @tx_sql += @nwl + '            RAISERROR(@tx_error_message, 18, 1)'
    SET @tx_sql += @nwl + '        END'
    SET @tx_sql += @nwl + '      END'
    SET @tx_sql += @nwl + '      '
