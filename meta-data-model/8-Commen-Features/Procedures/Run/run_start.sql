...
BEGIN
  ...
  IF (1=1 /* "Start" run. */) BEGIN
    ...
    IF (1=1 /* Check if @id_dataset is valid, extract dt_previous_stand */) BEGIN
      ...
      SET @tx_sql += @tx_emp + '  SELECT MAX(meta_dt_valid_from) AS dt FROM <@nm_target_schema>.<@nm_target_table> UNION ALL';
      SET @tx_sql += @tx_nwl + '  SELECT MAX(meta_dt_valid_till) AS dt FROM <@nm_target_schema>.<@nm_target_table> WHERE meta_dt_valid_till < CONVERT(DATE, "9999-12-31") UNION ALL ';
      SET @tx_sql += @tx_nwl + '  SELECT CONVERT(DATETIME, "1970-01-01") AS dt';
